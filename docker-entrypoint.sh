...
# Note: NOT using set -e to allow graceful fallback if mysql client is unavailable
...
if ! command -v mysql &> /dev/null; then
    echo "[Init] ⚠️  MySQL client not installed - skipping database initialization"
    echo "[Init] Application will attempt to connect at runtime"
else
