...
$conv_res = $conn->prepare("SELECT DISTINCT
                                CASE WHEN sender_user_id = ? THEN recipient_user_id ELSE sender_user_id END as other_user_id
                             FROM messages
                             WHERE sender_user_id = ? OR recipient_user_id = ?");
$conv_res->bind_param("iii", $current_user_id, $current_user_id, $current_user_id);
$conv_res->execute();
$conv_rows = $conv_res->get_result();

$conversations = [];
while ($row = $conv_rows->fetch_assoc()) {
    $u_stmt = $conn->prepare("SELECT id, name, roll_number FROM users WHERE id = ?");
    $u_stmt->bind_param("i", $row['other_user_id']);
    $u_stmt->execute();
    $conversations[] = $u_stmt->get_result()->fetch_assoc();
}
...
if ($to_id > 0) {
    $u_stmt = $conn->prepare("SELECT id, name, roll_number, branch FROM users WHERE id = ?");
    ...
    if ($active_recipient) {
        ...
        $m_stmt->execute();
        $m_rows = $m_stmt->get_result();
        while ($m = $m_rows->fetch_assoc()) $messages[] = $m;
    }
}
?>
                ...
                <a href="<?php echo BASE_URL; ?>/?page=messages&to=<?php echo $conv['id']; ?>"
                   class="flex items-center gap-4 p-4 rounded-2xl transition-all <?php echo ($to_id == $conv['id']) ? 'bg-white shadow-subtle border border-slate-100' : 'hover:bg-white/60'; ?>">
                    ...
                    <div class="flex-1 overflow-hidden">
                        ...
                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-tight"><?php echo sanitize($conv['roll_number']); ?></p>
                    </div>
                </a>
                <?php endforeach; ?>
                <?php if (empty($conversations)): ?>
                    <p class="text-xs text-slate-400 text-center mt-10 font-medium italic px-4">No active conversations.<br/>Join a track to start collaborating.</p>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="hidden md:flex flex-1 flex-col bg-white">
            <?php if ($active_recipient): ?>
                ...
                <div class="p-6 border-b border-slate-50 flex items-center justify-between">
                    ...
                    <button class="btn-outline !py-2 !px-4 !text-xs font-bold">View Track</button>
                </div>
                ...
                <div class="p-6 bg-white border-t border-slate-50">
                    <form action="<?php echo BASE_URL; ?>/src/controllers/messages.php?action=send" method="POST" class="flex gap-4">
                        ...
                        <input type="text" name="message" required class="form-input !rounded-2xl" placeholder="Write a professional message...">
            ...
            <?php else: ?>
                <div class="flex-1 flex flex-col items-center justify-center p-12 text-center opacity-40">
                    ...
                    <h3 class="text-xl font-bold text-slate-900">Select a connection</h3>
                    <p class="text-sm font-medium text-slate-500 mt-2">Choose a lead builder or collaborator to start the conversation.</p>
                </div>
            <?php endif; ?>
        </div>
    </div>
