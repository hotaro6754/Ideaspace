...
$res = $conn->prepare("SELECT collaborations.*, ideas.title, ideas.domain, users.name as leader_name
                        ...
                        WHERE collaborations.collaborator_id = ? AND collaborations.status = 'active'");
$res->bind_param("i", $user['id']);
$res->execute();
$collabs = [];
while ($row = $res->get_result()->fetch_assoc()) $collabs[] = $row;
?>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 animate-fade-up">
        ...
        <div class="premium-card p-8 bg-white border-l-4 border-l-green-500">
            <div class="flex items-center justify-between mb-6">
                ...
                <span class="text-[10px] font-black text-green-600 uppercase tracking-widest">Active Partner</span>
            </div>
            ...
            <div class="pt-6 border-t border-slate-50 flex gap-3">
                <a href="<?php echo BASE_URL; ?>/?page=messages&to=<?php echo $c['leader_id']; ?>" class="btn-primary !w-full !py-3 !text-[10px] uppercase tracking-widest">Team Chat</a>
