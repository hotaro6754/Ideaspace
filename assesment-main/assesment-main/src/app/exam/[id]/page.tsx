...
import { AlertCircle, ShieldAlert, CheckCircle2, Lock, Loader2, ChevronLeft, ChevronRight, AlertTriangle } from "lucide-react"
...
export default function ExamPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const initializeExam = async () => {
    ...
    // Check for existing session in localStorage for this exam
    const storageKey = `exam_session_${exam.id}_${user.uid}`
    ...
    if (existingSession) {
      ...
      if (Date.now() >= sessionData.endTime) {
         sessionData = null // Expired
    ...
    if (sessionData) {
      currentResultId = sessionData.resultId
      currentEndTime = sessionData.endTime
      setAnswers(sessionData.answers || {})
      setViolationCount(sessionData.violationCount || 0)
    } else {
      currentResultId = doc(collection(db, "users", user.uid, "results")).id
      currentEndTime = Date.now() + (exam.timeLimitMinutes * 60 * 1000)
      ...
      const resultRef = doc(db, "users", user.uid, "results", currentResultId)
      setDoc(resultRef, {
        id: currentResultId,
        studentId: user.uid,
  ...
  }, [answers, violationCount, isStarted, isFinished, resultId, db, user, exam])

  const finishExam = useCallback(async (forcedStatus?: string) => {
    if (isFinished || !resultId || !user || !exam) return
    setIsFinished(true)
    
    const finalStatus = forcedStatus || (violationCount > 0 ? 'Flagged' : 'Clean')
    ...
    localStorage.removeItem(`exam_session_${exam.id}_${user.uid}`)
  ...
  }, [isFinished, resultId, violationCount, db, user, exam, toast])
  ...
  useEffect(() => {
      ...
      setViolationCount(prev => {
        ...
        if (newCount >= MAX_VIOLATIONS) {
          toast({
            ...
            description: "Maximum violations reached. Auto-submitting exam.",
            variant: "destructive"
          })
          finishExam('Flagged')
        } else {
          toast({
            ...
            description: `Focus loss detected. Violation ${newCount} of ${MAX_VIOLATIONS}.`,
            variant: "destructive"
          })
        }
        return newCount;
    ...
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'hidden') handleProctoringViolation()
    }
  ...
  }, [isStarted, isFinished, resultId, db, toast, user, finishExam])

  // Absolute Timer Countdown Logic
  useEffect(() => {
    if (!isStarted || isFinished || !endTime) return
  ...
  }, [isStarted, isFinished, endTime, finishExam])
          ...
          <CardContent className="space-y-6">
            ...
            <div className="bg-destructive/10 p-4 rounded-lg border border-destructive/20 text-xs text-destructive flex gap-3">
              ...
              <div className="space-y-1">
                ...
                <p>3. {MAX_VIOLATIONS} violations will result in automatic submission.</p>
                <p>4. The test will auto-submit exactly when the timer hits zero (server-synced).</p>
              </div>
            </div>
          </CardContent>
          <CardFooter>
        ...
        <Card className="max-w-xl w-full glass-card">
          ...
          <CardContent className="space-y-4">
            <Alert variant={violationCount > 0 ? "destructive" : "default"}>
              ...
              <AlertTitle>Integrity Status: {violationCount > 0 ? 'Flagged' : 'Secure'}</AlertTitle>
              <AlertDescription>
                {violationCount > 0 
                  ? `Proctoring violations detected (${violationCount}/${MAX_VIOLATIONS}). This attempt is under review.` 
    ...
    <div ref={containerRef} className="min-h-screen bg-background select-none">
      <header className="border-b bg-card sticky top-0 z-50">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            {violationCount > 0 && (
              <Badge variant="destructive" className="animate-pulse gap-1 font-bold px-3 py-1 text-xs">
